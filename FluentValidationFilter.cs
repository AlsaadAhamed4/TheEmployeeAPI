...
public class FluentValidationFilter : IAsyncActionFilter
{
    ...
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)  // events once the action is executed.
    {
        foreach (var parameter in context.ActionDescriptor.Parameters)
        {
            if (context.ActionArguments.TryGetValue(parameter.Name, out var argumentValue) && argumentValue != null)
            {
                ...
                if (validator != null)
                {
                    ...
                    {
                        validationResult.AddToModelState(context.ModelState);  // we need a nugget package for this.
                        var problemDetails =
    ...
    public void OnActionExecuted(ActionExecutedContext context) { }  // after executed the action
}